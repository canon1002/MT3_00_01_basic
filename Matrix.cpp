...
/// アフィン変換行列を作成する
Matrix3x3 Matrix::MakeAffine(Vector2 scalar, float theta, Vector2 translate) {
...
/// 転置行列の計算
Matrix3x3 Matrix::Transpose(Matrix3x3 matrix) {
...
/// 正射影行列の作成
Matrix3x3 Matrix::MakeOrthographic(float left, float top, float right, float bottom) {
...
/// ビューポート行列の作成
Matrix3x3 Matrix::MakeViewport(float left, float top, float width, float height) {
...
Vector2 Matrix::Transform(Vector2 vector, Matrix3x3 matrix) {
...
Quad Matrix::Transform(Quad quad, Matrix3x3 matrix) {

	// 4か所それぞれで平行移動をおこなう
	quad.leftTop = Matrix::Transform(quad.leftTop, matrix);
	quad.rightTop = Matrix::Transform(quad.rightTop, matrix);
	quad.leftBottom = Matrix::Transform(quad.leftBottom, matrix);
	quad.rightBottom = Matrix::Transform(quad.rightBottom, matrix);
